	...
	<div class="stylePaiInfo">
		...
		<div class="info">
			<div class="top">
				...
				<h1>全屋设计套装</h1>
				<h2><span v-if="designInfo.stylePaiCode"><span v-show="designInfo.stylePaiCode=='unusual'">与众不同 · </span></span><span v-show="designInfo.stylePaiCode=='internetCelebrity'">时尚网红 · </span><span v-show="designInfo.stylePaiCode=='classicStyle'">经典风格 · </span></span>{{ designInfo.designName }}</h2>
		...
		<div class="location" :style="'height:'+screenHeight+'px;background: url('+selectLocation[selectLocationIndex].coverPic+') no-repeat center center;'" v-if="selectLocation.length">
				...
				<div class="box">
					<ul :style="'height:'+(locationList.length*1.52+0.76)+'rem'">
						<li v-for="(item,index) in locationList" :key='index' @click="selectLocationFun(index)">
							<img :src="item.templateList[0].coverPic" :alt="'空间'+(index+1)" />
			...
			<div class="location_preBtn" @click="changePic(false)" v-if="selectLocation.length>1">
	  		...
	  		<div class="locationInfo">
	  			<p>{{ designInfo.designName }} | {{ locationList[locationIndex].areaTypeName }}</p>
		...
		<div class="goodsInfo" v-if="goodsInfoList.length>0">
			<h1><span v-if="designInfo.stylePaiCode"><span v-show="designInfo.stylePaiCode=='unusual'">与众不同 · </span></span><span v-show="designInfo.stylePaiCode=='internetCelebrity'">时尚网红 · </span><span v-show="designInfo.stylePaiCode=='classicStyle'">经典风格 · </span></span>{{ designInfo.designName }}</h1>
			<!--<h2>基础套餐</h2>-->
			<div class="goodsList">
				<ul>
				...
				<div class="priceInfo">
					<div class="price" v-show="packagePrice">
						<span style="font-size: 0.17rem;">￥</span>{{ packagePrice }} <span style="font-size: 0.17rem;">起</span>
					</div>
					<div class="price" v-show="!packagePrice">
					...
					<div class="box">
						<ul :style="'width:'+styleList.length*3.35+'rem'">
							<li v-for="(item,index) in styleList" :key="index" :class="index==selectStyle?'active':''" @mouseover="selectStyle=index" @click="changeStyle(item)">
								...
								<div class="name">{{ item.designName }}</div>
...
export default {
	...
	methods:{
		...
		selectLocationFun(index){
			...
			let list=this.locationList[index].templateList;
		...
		changeStyle(obj){
			//console.log(obj)
			let code=Base64.encode(obj.uuid);
...
function getDesignInfo(obj){
	...
	obj.$ajax(obj.$store.state.localIP+'stylePai/getDesign','get',data,function(res){
		//console.log(res)
		obj.designInfo=res.data;
		obj.introduce.bgImg=res.data.coverPic;
		obj.introduce.p2=res.data.designName;
		obj.introduce.p3=res.data.address;
		//获取商品
		getGoodList(obj);
		//获取更多
		getMoreStyle(obj);
...
function getLocationInfo(obj){
	...
	obj.$ajax(obj.$store.state.localIP+'stylePai/getSpaceTemplateHaving','get',data,function(res){
		//console.log(res,2)
		if(res.data){			
			obj.locationList=res.data;
			if(res.data.length>0){
				let list=res.data[0].templateList ||[];
				for(let i=0;i<list.length;i++){
					if(list[i].details.length>50){
						list[i].details=list[i].details.substring(0,50);
					}
...
function getGoodList(obj){
	...
	obj.$ajax(obj.$store.state.localIP+'queryDesignGoodsList','GET',{designId:obj.designInfo.id},function(res){
		//console.log(res)
		let arr=[];
				...
				for(let j=0;j<goodsTypeList.length;j++){
					if(goodsTypeList[j].goodsInfos){
						...
						for(let x=0;x<goodsInfos.length;x++){
							if(arr.length<4){		
								if(goodsInfos[x].goodsImages.indexOf(',')>-1){									
									let imgList=goodsInfos[x].goodsImages.split(',');
									goodsInfos[x].goodsImages=imgList[0]
								}
...
function getPrice(obj){
	...
	obj.$ajax(obj.$store.state.localIP+'stylePai/getDesignFurnitureTotal','get',data,function(res){
		//console.log(res,3)
		obj.packagePrice=res.data || 0;
	})
}
//更多风格
function getMoreStyle(obj){
	var data={
      	excludeDesignUuid:obj.code,
		stylePaiCode:obj.designInfo.stylePaiCode
    }; 
